...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enforce friendship — non-friends must use message requests
    const isFriend = await prisma.friendship.findFirst({
      where: {
        OR: [
          { requesterId: currentUserId, recipientId, status: 'accepted' },
          { requesterId: recipientId, recipientId: currentUserId, status: 'accepted' },
        ],
      },
    })

    // Check if a prior accepted message request exists (unlocks DMs for non-friends)
    const acceptedRequest = isFriend
      ? true
      : await prisma.messageRequest.findFirst({
          where: {
            OR: [
              { senderId: currentUserId, recipientId, status: 'accepted' },
              { senderId: recipientId, recipientId: currentUserId, status: 'accepted' },
            ],
          },
        })
    ...
    if (!isFriend && !acceptedRequest) {
      return NextResponse.json(
        { error: 'MESSAGE_REQUEST_REQUIRED', message: 'Send a message request first.' },
        { status: 403 }
      )
    }
