...
export default function MessagesPage() {
  ...
  const [msgReqActioningId, setMsgReqActioningId] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    async function load(options?: { silent?: boolean }) {
      ...
      try {
        ...
        const [conversationData, friendRequestData, msgReqData] = await Promise.all([
          fetchConversations(abortController.signal),
          fetchFriendRequests(abortController.signal),
          fetch('/api/message-requests', { signal: abortController.signal })
        ...
        if (!cancelled) {
          ...
          setMsgRequests(msgReqData.requests ?? [])
        }
      } catch (error) {
  ...
  async function handleMsgRequestDecision(requestId: string, action: 'accept' | 'decline') {
    try {
      setMsgReqActioningId(requestId)
      ...
      setMsgRequests((prev) => prev.filter((r) => r.id !== requestId))
    } catch {
      // swallow — keep request visible
    } finally {
      setMsgReqActioningId(null)
    }
  }
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#090b10] text-stone-100">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl gap-6 px-4 pb-6 pt-24 sm:px-6 lg:px-8">
        ...
        <main className="flex-1 space-y-5">
          ...
          {!isLoading && !loadError && msgRequests.length > 0 && (
            <section className="rounded-3xl border border-white/10 bg-black/20 p-4 backdrop-blur-lg sm:p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.22em] text-sky-200/70">Messages</p>
                  <h3 className="mt-2 font-[family:var(--font-display)] text-2xl text-sky-100">
                    Message requests
                  </h3>
                </div>
                <p className="text-xs uppercase tracking-[0.14em] text-stone-400">
                  {msgRequests.length} pending
                </p>
              </div>
              <ul className="mt-4 space-y-3">
                {msgRequests.map((req) => (
                  <li
                    key={req.id}
                    className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:flex-row sm:items-start"
                  >
                    <div className="flex min-w-0 flex-1 items-start gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 text-sm font-bold text-stone-300 overflow-hidden">
                        {req.sender.avatarUrl ? (
                          <img src={req.sender.avatarUrl} alt="" className="h-10 w-10 object-cover" />
                        ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-stone-500">@{req.sender.username} · {formatRelativeTime(req.createdAt)}</p>
                        {req.intro && (
                          <p className="mt-2 text-sm text-stone-300 leading-6 line-clamp-3">{req.intro}</p>
                        )}
                      </div>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button
                        type="button"
                        disabled={msgReqActioningId === req.id}
                        onClick={() => void handleMsgRequestDecision(req.id, 'decline')}
                        className="rounded-xl border border-white/15 px-3 py-2 text-xs text-stone-400 transition hover:border-white/25 hover:text-stone-200 disabled:cursor-wait disabled:opacity-50"
                      >
                        Decline
                      </button>
                      <button
                        type="button"
                        disabled={msgReqActioningId === req.id}
                        onClick={() => void handleMsgRequestDecision(req.id, 'accept')}
                        className="rounded-xl border border-sky-400/30 bg-sky-500/10 px-3 py-2 text-xs font-semibold text-sky-300 transition hover:border-sky-300/40 hover:text-sky-200 disabled:cursor-wait disabled:opacity-50"
                      >
                        {msgReqActioningId === req.id ? '…' : 'Accept'}
                      </button>
                    </div>
                  </li>
                ))}
