...
const STEPS: StepConfig[] = [
  ...
  },
  {
    key: 'preferences',
    title: 'Preferences',
    subtitle: "What are you looking for? Pick all that apply.",
  },
  {
    key: 'desires',
    title: 'Desires',
...
function OnboardingContent() {
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    if (stepIndex === 4 && formData.lookingFor.length === 0) {
      nextErrors.lookingFor = 'Please select at least one option'
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await onboard({
        ...
        intentions: formData.intentions || undefined,
    ...
    if (currentStep === 3) {
      return (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700">Pronouns <span className="font-normal text-slate-500">(optional)</span></label>
            <select
              value={formData.pronouns}
              onChange={(event) => setFieldValue('pronouns', event.target.value)}
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition focus:border-[#8c1f43] focus:ring-2 focus:ring-[#d4b16a]/35"
            >
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700">What brings you here? <span className="font-normal text-slate-500">(optional)</span></label>
            <select
              value={formData.intentions}
              onChange={(event) => setFieldValue('intentions', event.target.value)}
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition focus:border-[#8c1f43] focus:ring-2 focus:ring-[#d4b16a]/35"
            >
